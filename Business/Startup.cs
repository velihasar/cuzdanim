...
namespace Business
{
    public partial class BusinessStartup
    {
        ...
        public virtual void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddHttpClient<Business.Services.IAssetTypePriceService, Business.Services.AssetTypePriceService>();
            
            // HttpClient for Turnstile verification
            services.AddHttpClient("Turnstile");
            services.AddScoped<Core.Utilities.Security.Turnstile.TurnstileHelper>(sp =>
            {
                var httpClientFactory = sp.GetRequiredService<IHttpClientFactory>();
                var httpClient = httpClientFactory.CreateClient("Turnstile");
                var configuration = sp.GetRequiredService<IConfiguration>();
                return new Core.Utilities.Security.Turnstile.TurnstileHelper(configuration, httpClient);
            });
            ...
            if (taskSchedulerConfig.Enabled)
            {
                services.AddHangfire(config =>
                {
                    ...
                    // Recurring job'lar [RecurringJob] attribute ile otomatik register edilir
                    // JSON dosyasından job yükleme (opsiyonel)
                ...
                {
                    options.Queues = new[] { "default", "jobs" }; // "jobs" queue'sunu da işle
